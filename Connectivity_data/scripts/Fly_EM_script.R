...
adj_mtx_gFlyS  <- igraph::as_adjacency_matrix(gFlyS, attr = "weight")
...
#routing efficiency
#am_gFlyS  <- get.adjacency(gFlyS, attr = "weight")
...
E(er_Fly)$weight <- E(gFlyS)$weight
...
#modularity y comunitats
adj_mtx_er_Fly  <- igraph::as_adjacency_matrix(er_Fly, attr = "weight")
...
#routing efficiency
#am_er_Fly  <- get.adjacency(er_Fly, attr = "weight")
...
sw_Fly <- rewire(graph.lattice(length(unique(archiuFly[,1])), nei=11, dir=TRUE, mutual=TRUE), 
                 with = each_edge(p = .02 , loops = FALSE))
pesos_sw_Fly <- sample(E(gFlyS)$weight,length(E(sw_Fly)))
E(sw_Fly)$weight <- pesos_sw_Fly
...
#modularity y comunitats
adj_mtx_sw_Fly <- igraph::as_adjacency_matrix(sw_Fly, attr = "weight")
...
#routing efficiency
#am_sw_Fly  <- get.adjacency(sw_Fly, attr = "weight")
...
pesos_ba_Fly <- sample(E(gFlyS)$weight,length(E(ba_fly)))
E(ba_fly)$weight <- pesos_ba_Fly
...
#modularity y comunitats
adj_mtx_ba_fly <- igraph::as_adjacency_matrix(ba_fly, attr = "weight")
...
#routing efficiency
#am_ba_fly  <- get.adjacency(ba_fly, attr = "weight")
...
FlyEM_BA_data[["communities_FlyEM_BA"]] <- camunitats_ba_fly  
...
load("~/BNO/Connectivity_data/FlyEM_data.RData")

write(FlyEM_data[["nNodes_FlyEM"]], file = "networkFlyEM.dat", sep = " ")

dadesDeArchiuFlyEM <- get.data.frame(gFlyS)
dadesDeArchiuFlyEM$from <- as.numeric(as.factor(dadesDeArchiuFlyEM$from))
dadesDeArchiuFlyEM$to <- as.numeric(as.factor(dadesDeArchiuFlyEM$to))
dadesDeArchiuFlyEM$name <- NULL

write.table(dadesDeArchiuFlyEM, file = "networkFlyEM.dat", sep = " ",append = TRUE,
            col.names = F,
            row.names = FALSE)

#---

load("~/BNO/Connectivity_data/FlyEM_ER_data.RData")

write(FlyEM_ER_data[["nNodes_FlyEM_ER"]], file = "networkFlyEM_ER.dat", sep = " ")

dadesDeArchiuFlyEM_ER <- get.data.frame(er_Fly)
dadesDeArchiuFlyEM_ER$from <- as.numeric(as.factor(dadesDeArchiuFlyEM_ER$from))
dadesDeArchiuFlyEM_ER$to <- as.numeric(as.factor(dadesDeArchiuFlyEM_ER$to))
dadesDeArchiuFlyEM_ER$name <- NULL

write.table(dadesDeArchiuFlyEM_ER, file = "networkFlyEM_ER.dat", sep = " ",append = TRUE,
            col.names = F,
            row.names = FALSE)

#-----

load("~/BNO/Connectivity_data/FlyEM_SW_data.RData")

write(FlyEM_SW_data[["nNodes_FlyEM_SW"]], file = "networkFlyEM_SW.dat", sep = " ")

dadesDeArchiuFlyEM_SW <- get.data.frame(sw_Fly)
dadesDeArchiuFlyEM_SW$from <- as.numeric(as.factor(dadesDeArchiuFlyEM_SW$from))
dadesDeArchiuFlyEM_SW$to <- as.numeric(as.factor(dadesDeArchiuFlyEM_SW$to))
dadesDeArchiuFlyEM_SW$name <- NULL

write.table(dadesDeArchiuFlyEM_SW, file = "networkFlyEM_SW.dat", sep = " ",append = TRUE,
            col.names = F,
            row.names = FALSE)


#-----
load("~/BNO/Connectivity_data/FlyEM_BA_data.RData")

write(FlyEM_BA_data[["nNodes_FlyEM_BA"]], file = "networkFlyEM_BA.dat", sep = " ")

dadesDeArchiuFlyEM_BA <- get.data.frame(ba_fly)
dadesDeArchiuFlyEM_BA$from <- as.numeric(as.factor(dadesDeArchiuFlyEM_BA$from))
dadesDeArchiuFlyEM_BA$to <- as.numeric(as.factor(dadesDeArchiuFlyEM_BA$to))
dadesDeArchiuFlyEM_BA$name <- NULL

write.table(dadesDeArchiuFlyEM_BA, file = "networkFlyEM_BA.dat", sep = " ",append = TRUE,
            col.names = F,
            row.names = FALSE)